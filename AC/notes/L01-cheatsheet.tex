...
% ── Prevent math overflow in boxes: shrink-to-fit for display math ──────
% Usage: \fitmath{L = \{w \in \Sigma^* \mid \text{very long condition}\}}
...
\fitmath{O(\sqrt{n}) = O(\sqrt{2^k}) = O(2^{k/2})}
...
\textbf{Example from the slide:}
\fitmath{\mathbb{B}^* = \{\varepsilon, 0, 1, 00, 01, 10, 11, 000, 001, 010, 011, 100, \ldots\}}
...
\textbf{Union:}
\fitmath{L_1 \cup L_2 = \{w \in \Sigma^* \mid w \in L_1 \text{ or } w \in L_2\}}
Take all words that are in either language.

\textbf{Intersection:}
\fitmath{L_1 \cap L_2 = \{w \in \Sigma^* \mid w \in L_1 \text{ and } w \in L_2\}}
Take only words that are in \textit{both} languages.

\textbf{Complement} (with respect to $\Sigma^*$):
\fitmath{\overline{L_1} = \{w \in \Sigma^* \mid w \notin L_1\}}
All words that are NOT in $L_1$.

\textbf{Concatenation:}
\fitmath{L_1 \cdot L_2 = \{w = w_1 w_2 \mid w_1 \in L_1 \text{ and } w_2 \in L_2\}}
All words you can make by concatenating one word from $L_1$ with one from $L_2$.

\textbf{Kleene star (iteration):}
\fitmath{(L_1)^* = \{w = w_1 w_2 \cdots w_k \mid k \geq 0,\; w_i \in L_1 \text{ for all } i\}}
...
\fitmath{L_{\text{prime}} = \{w \in \{0, 1, 2, \ldots, 9\}^* \mid w \text{ is the decimal representation of a prime}\}}
\textit{Example:} ``17'' $\in L_{\text{prime}}$. ``15'' $\notin L_{\text{prime}}$.

\textbf{2. ``Does a graph have a path from vertex 1 to vertex 2?''}
\fitmath{L_{\text{path}} = \{w \in \mathbb{B}^* \mid w \text{ encodes adj.\ matrix with path from vertex 1 to 2}\}}
...
\fitmath{L_{\text{SAT}} = \{w \in \Sigma_L^* \mid w \text{ encodes a satisfiable formula}\}}
\textit{This is THE central problem of NP-completeness (Lecture 6).}

\textbf{4. ``Does a given program ever output \texttt{False}?''}
\fitmath{L_{\text{output}} = \{w \in \{a, b, c, \ldots\}^* \mid w \text{ is Python source that outputs \texttt{False} for some input}\}}
...
\begin{center}
\begin{tikzpicture}[->, >=Stealth, node distance=2.5cm, every state/.style={thick, fill=blue!5, minimum size=0.9cm}]
...
\textit{Read the diagram: start at $q_0$. Reading 0s stays in $q_0$. Reading a 1 moves to $q_1$ (double circle = accept). Any further input goes to $q_2$ (reject sink --- no escape).}
...
\textbf{State diagram} (showing the main transitions for matching a 1):

\begin{center}
\begin{tikzpicture}[->, >=Stealth, node distance=2.8cm, every state/.style={thick, minimum size=0.9cm}]
    \node[state, initial, fill=blue!8]          (qr)  {$q_r$};
    \node[state, fill=orange!10]                (qs)  [right of=qr] {$q_s$};
    \node[state, fill=red!8]                    (ql1) [below of=qs] {$q_\ell^1$};
    \node[state, fill=red!15]                   (qa1) [left of=ql1] {$q_a^1$};
    \node[state, accepting, fill=green!10]      (t)   [below of=qa1] {$t$};

    \path (qr)  edge [loop above] node {\scriptsize $0/0,R$\; $1/1,R$\; $\#/\#,R$} ()
          (qr)  edge              node [above] {\scriptsize $\sqcup/\sqcup,L$} (qs)
          (qs)  edge              node [right] {\scriptsize $1/0,L$} (ql1)
          (ql1) edge [loop right] node {\scriptsize $0/0,L$\; $1/1,L$} ()
          (ql1) edge              node [below] {\scriptsize $\#/\#,L$} (qa1)
          (qa1) edge              node [left]  {\scriptsize $1/0,R$} (t)
          (qa1) edge [bend left]  node [left]  {\scriptsize match$\to q_r$} (qr);
\end{tikzpicture}
\end{center}

\textit{Blue = scanning phase, orange = reading last symbol, red = carrying \& matching, green = accept. Arrows show: read/write, direction.}
...
\fitmath{\delta(q, a) = (q', a', d) \;\;\text{means}\;\; \text{``If state } q \text{ reading } a\text{: go to } q'\text{, write } a'\text{, move } d\text{''}}
...
\begin{tikzpicture}[->, >=Stealth, node distance=2.8cm, every state/.style={thick, minimum size=0.9cm}]
    ...
    \node[state, accepting, fill=green!10]  (t)  [above right of=q0] {$t$};
    \node[state, fill=red!10]               (r)  [below right of=q0] {$r$};

    \path (q0) edge [loop above] node {\scriptsize $0/0, R$} ()
          (q0) edge              node [above left] {\scriptsize $1/1, R$} (t)
          (q0) edge              node [below left] {\scriptsize $\sqcup/\sqcup, R$} (r);
\end{tikzpicture}
\end{center}

\textit{Simple: stay in $q_0$ scanning 0s. See a 1 $\to$ accept. See blank $\to$ reject (no 1 found).}
...
\fitmath{\tau(n) = \begin{cases} 0 & \text{if } n \leq 5 \\ 1 & \text{if } n = 6 \\ \sqcup & \text{if } n \geq 7 \end{cases}}
...
\fitmath{\tau = \tau(1)\tau(2)\cdots\tau(n_0)\sqcup\ldots}
...
\fitmath{\alpha_w = [s, w\sqcup\ldots, 1]}
...
\fitmath{\tau'(n) = \begin{cases} a & \text{if } n = \ell \;\text{(write } a \text{ at head position)} \\ \tau(n) & \text{otherwise} \;\text{(unchanged)} \end{cases}}
...
\begin{center}
\begin{tikzpicture}[->, >=Stealth, node distance=2.8cm, every state/.style={thick, minimum size=0.9cm}]
    \node[state, initial, fill=yellow!15]       (q0) {$q_0$};
    \node[state, accepting, fill=green!10]      (t)  [above right of=q0] {$t$};
    \node[state, fill=red!10]                   (r)  [below right of=q0] {$r$};

    \path (q0) edge [loop above] node {\scriptsize $0/0,R$\;\; $1/1,R$\;\; $\sqcup/\sqcup,R$} ();
    % No arrows to t or r! That's why it loops.
\end{tikzpicture}
\end{center}

\textit{Notice: there are \textbf{no arrows to $t$ or $r$}. Every symbol loops back to $q_0$. The machine can never stop.}
...
\fitmath{L(M) = \{w \in \Sigma^* \mid M \text{ accepts } w\}}
